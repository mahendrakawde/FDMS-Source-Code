...
public class ProcessCaseListOptions extends Action {
    ...
    public ActionForward execute(ActionMapping mapping,
        ...
        try {
           ...
           // t = (com.aldorsolutions.webfdms.database.DatabaseTransaction)DatabaseTransaction.getTransaction(sessionUser);
        	String dbLookup = UtilSingleton.getInstance().getUserDBLookup();
        	
            t = (DatabaseTransaction) DatabaseTransaction.getTransaction( sessionUser, dbLookup );
            t.setReadOnly(false);
        ...
        if (!errors.isEmpty() ) {
            // AppLog.info("ProcessCaseListOptions Invoking forward mapping getInput().");
        ...
        // AppLog.trace("ProcessCaseListOptions Invoking forwardReturnShowCaseList.");
        return actionForward;
