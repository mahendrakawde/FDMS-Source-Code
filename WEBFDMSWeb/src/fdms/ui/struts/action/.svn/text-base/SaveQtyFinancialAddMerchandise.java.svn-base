...
public class SaveQtyFinancialAddMerchandise extends Action {
    ...
    public ActionForward execute(
        ...
        QtyFinancialMerchandiseInformationForm form = (QtyFinancialMerchandiseInformationForm) actionForm;
...
//                for (int i = 0; i < (form.getLineItems().size());i++) {
                ...
                String[] SNs	  = form.getSerialNumbers();
                String[] onHandData;
                String[] onHandofTheMaster;
                String onHandID, masterID, SN;
                int qtyOfonhand = 0;
                int qtyNumber = 0;
...
//                String[] serialItems = form.getSerialItems();
//                String[] keys = form.getKeys();
                ...
                while(myIterator.hasNext()) {
                	...
                	int priceId = aCharge.getDbChargeItem().getInvSeqNo();
//                    //Retreive A PriceList Record
                    ...
                    qtyOfonhand = 0;
                    if (SNs != null) {
	                    onHandofTheMaster = new String [SNs.length];
	                    for (int i=0; i< SNs.length; i++) {
	                    	SN = SNs[i];
	                    	onHandData = SN.split("-");
	                    	onHandID = onHandData[0];
	                    	masterID = onHandData[1];
	                    	if (masterID.compareTo(Integer.toString(aCharge.getDbChargeItem().getInvSeqNo()))==0) {
	                    		onHandofTheMaster[qtyOfonhand] = onHandID;
	                    		qtyOfonhand++;  
	                    	}
	                    }
                    }
                    else {
                    	onHandofTheMaster = new String [1];
                    	qtyOfonhand= 0;
                    }
                    ...
                    if (qtyOfonhand > 0) {
                    	qtyNumber = qtyOfonhand;
                    } else {
                    	qtyNumber = aCharge.getItemQty();
                    }
                    
                    if (priceList.getCStockType().equalsIgnoreCase(DbInvMaster.STOCK_TYPE_SERIAL) && qtyOfonhand == 0 && qtyNumber == 0){  
                    	qtyNumber = 0;
                    	
                    }
                    
                    
                    for (int i=0; i< qtyNumber; i++) {
	                    ...
	                    if (priceList == null){
	                        ...
	                        int uniqueSeqNo = SessionHelpers.calculateSequenceNumber(chargeSetFin, dbChargeItem);
	                        ...
	                        if (taxCodes[a].compareToIgnoreCase("Default")==0){
	                        	dbChargeItem.setTaxCode(priceList.getCTaxCode());
	                        } else {
	                        ...
	                        if (priceList.getCStockType().equalsIgnoreCase(DbInvMaster.STOCK_TYPE_SERIAL)){                            
	                            ...
	                            if (aCharge.getItemQty()==0 && qtyOfonhand == 0) {
	                            	collectionItem.setSerialNumber("- Select -");
	                            }
	                            else {
		                            if (onHandofTheMaster[i].compareTo("0")==0){
		                            	collectionItem.setSerialNumber("- Select -");
		                            }
		                            else {
		                            	DbInvOnHand invOnHand = FdmsDb.getInstance().getInvOnHand(t, Integer.valueOf(onHandofTheMaster[i]));
		                            	collectionItem.setSerialNumber(invOnHand.getCSerialNumber());
		                            	collectionItem.getDbChargeItem().setInvOnHandID(Integer.valueOf(onHandofTheMaster[i]));
		                            }
	                            }
	                        ...
	                        if (taxCodes[a].compareToIgnoreCase("Default")==0){
	                        	collectionItem.setItemTaxCode(priceList.getCTaxCode());
	                        } else {
                ...
                //financialInfo.setLineItems(chargeSetFin);
                qtyFinancialMerchandiseInfo.setLineItems(chargeSetFin);
