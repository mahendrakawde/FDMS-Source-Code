...
public class ShowAcctInterface extends Action {
    
    private Logger logger = Logger.getLogger(ShowAcctInterface.class.getName());
    ...
    public ActionForward execute(ActionMapping mapping,
        ...
        AcctIntfForm acctform = new AcctIntfForm();
...
System.out.println("http://" + httpURL.getHost() + "/" + InterfaceAccounting.getHttpBaseDir((DbUser)sessionUser)+diskfile.getName());
        ...
        request.setAttribute("acctIntfForm",acctform) ;
        return mapping.findForward("showAcctInterfaceJsp");
    }
    
    private void createQWCTemplate (HttpServletRequest request, DbUserSession user, AcctIntfForm form) {
    	// {0} = URL
    	// {1} = userName
