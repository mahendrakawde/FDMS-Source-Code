...
/**
 * DbMemorialSetPeer provides the runtime class name for the Persistent objects
 * it is responsible for as well as the SQL for restoring them.
 * Creation date: (4/30/2002 11:16:20 AM)
 * @author: 
 */
public class DbMemorialSetPeer extends com.aldorsolutions.webfdms.database.DatabaseSetPeer {
/**
 ...
 * If a "Locale" is passed, then all memorials for a locale code
 * are provided.
...
public String getSql(java.util.Hashtable h) {
	if( h == null ) { 
		...
		return "SELECT * FROM memorial order by description";
	}
	else {
		StringBuffer tmp = new StringBuffer();
		tmp.append("SELECT * FROM memorial where (Locale=0");

		if( h.containsKey(DbMemorialPeer.LOCALE) ) {
			// Get records for one Locale
			tmp.append(" OR Locale = ");
			tmp.append(h.get(DbMemorialPeer.LOCALE).toString());
		}
		tmp.append(")");
		tmp.append(" ORDER BY description");
		return tmp.toString();
	}
}
}
