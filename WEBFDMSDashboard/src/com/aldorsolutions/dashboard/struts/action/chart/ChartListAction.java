...
public class ChartListAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		ArrayList <UserLocaleDTO> locales = uMgr.getLocalesOfUser( user.getDbLookup(), (int) user.getId(),user.getCompanyID(), user.getRegion());
		ArrayList <UserLocaleDTO> rUserLocales = new ArrayList<UserLocaleDTO>();
		for (UserLocaleDTO locale: locales){
        	for (UserLocaleDTO userLocale: locales ){
        ...
        */
		UserManagerBean uMgr = new UserManagerBean();
		ArrayList <UserLocationDTO> locations = uMgr.getLocationsOfUser( user.getDbLookup(), (int) user.getId(),user.getCompanyID(), user.getRegion() ); 
		ArrayList <UserLocaleDTO> locales = uMgr.getLocalesOfUser( user.getDbLookup(), (int) user.getId(), user.getCompanyID(), user.getRegion());
		...
		session.setAttribute("userLocales", rUserLocales);
		
		Date aDate= new Date();
		 if(chartListForm.getFromDate()==null){
			 chartListForm.setFromDate("01/01/"+FormatDate.convertDateToYYYYString(aDate));
		 } 
		 
		 if( chartListForm.getToDate() == null){
			chartListForm.setToDate(FormatDate.getCurrentDateFormatedMMDDYYYY());
		 }
		session.setAttribute("fromDate", chartListForm.getFromDate());
		session.setAttribute("toDate", chartListForm.getToDate());
		session.setAttribute("localeID", chartListForm.getLocaleID());
		 
		//System.out.println("From date : "+ chartListForm.getFromDate());
		//System.out.println("To date : "+ chartListForm.getToDate());
		//System.out.println("Locale : "+ chartListForm.getLocaleID());
