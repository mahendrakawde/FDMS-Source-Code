...
<security:token hasRole="Vendor Menu: Edit">
	...
	<html:form action="/acctSaveVendors">
		...
		--%>
			...
			<tr>
				<td colspan="2"><br /> <br /> <bean:define id="vendorLocs"
						name="ADMIN_VENDOR_LOCATIONS" scope="session"
						type="java.util.ArrayList" /> <logic:notEmpty name="vendorLocs">
						<fieldset class="">
							<legend>Vendor Locations: &nbsp; &nbsp; </legend>
							<table>
								<tr  class="trHead">
									<th class="trSep">Edit</th>
									<th class="trSep">Locale</th>
									<th class="trSep">Location</th>
									<th class="trSep">Default Account</th>
									<th class="trSep">Remove</th>
								</tr>
								<logic:iterate id="vendorLoc" name="vendorLocs">
									<tr>
										<td class="tdTable"><html:link action="/vendorLocEdit"
												paramName="vendorLoc" paramId="vendorLocationID"
												paramProperty="vendorLocationID" styleClass="editImage">
												
									</html:link></td>
										<td class="tdTable">${vendorLoc.localeName}</td>
										<td class="tdTable">${vendorLoc.locationName}</td>
										<td class="tdTable">${vendorLoc.defaultAcct}</td>
										<%-- 
								<td>
								...
								--> <input type="button" 
											onclick="checkRemoveLocation(${vendorLoc.vendorLocationID})" class="imageRemove">
			...
			<tr>
				<td colspan="2"><c:if
						test="${acctEditVendorsForm.vendorID <= 0 }">
						<div id='newVendorLoc' style="display: ">
					</c:if> <c:if test="${acctEditVendorsForm.vendorID > 0 }">
						<a href="javascript:toggle('newVendorLoc');">Add New Vendor
							Location</a>
					...
					<fieldset>
						...
						<table>
							<logic:greaterEqual name="acctEditVendorsForm"
								property="vendorID" value="0">
										...
										<table width="100%" cellpadding="3" cellspacing="0" border="0">
											<tr class="verdana12">
												<td class="label" align="right" nowrap="nowrap">
													Locale:</td>
												<td><html:select property="localeIds"
														styleClass="input" onchange="javascript:selectLocale();">
											...
											<tr class="verdana12">
												<td class="label" align="right" nowrap="nowrap">
													Location:</td>
												<td><html:select property="locationIds"
														styleClass="input" onchange="javascript:selectLocation();">
														<html:options collection="locationDtos"
											...
											<c:if test="${acctEditVendorsForm.vendorID > 0 }">
												<tr>
													<td colspan="2" align="center"><html:submit
															value="Add Location" property="submitType" styleClass="verdana12" /></td>
												</tr>
											</c:if>
										</table></td>
								</tr>
			...
			<tr>
				<td colspan="2"><html:submit value="Submit"
						property="submitType" onclick="return checkvalue('Submit')" styleClass="verdana12" /> <html:submit
						value="Inactive" property="submitType"
						onclick="return checkvalue('Delete')"  styleClass="verdana12" /> <html:cancel
						value="Cancel" property="submitType"
						onclick="return cancelConfirmation()"  styleClass="verdana12" /></td>
			</tr>
			<!--  
		<tr>
			<td>
